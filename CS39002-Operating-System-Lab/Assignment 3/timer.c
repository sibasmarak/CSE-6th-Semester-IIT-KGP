...
timer_init (void) 
{
  ...
  // initialize sleeping_threads list
...
timer_sleep (int64_t ticks)
{
  ...
  // check that currently interrupt is enabled
  ASSERT (intr_get_level () == INTR_ON);
  // disable interrupts
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  // traverse through sleeping_threads list upto the elem whose wake_tick >= ticks
  for (curr = list_begin(&sleeping_threads); curr!=list_end(&sleeping_threads); curr=list_next(curr))
  {
    ...
    if (curr_thread->wake_tick > ticks)
      break;
